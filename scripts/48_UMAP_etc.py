...
# for bin in [top_bins[0]]:
    ...
    # add contig length
    contig_len = df_abund_tmp['contig'].str.split("length_", n = 1, expand = True)[1].str.split("_cov", n=1, expand=True)[0]
    df_abund_tmp['length'] = list(map(int, contig_len))
    ...
    if len(df_abund_tmp['contig']) <= 1:
        ...
        try :
            ...
            for contig in df_mahal['contig']:
                df_mahal.loc[df_mahal['p-val'] <= pval,'outlier']=1           
            ...
            # total_outliers = len(df_mahal[df_mahal['outlier']==1])
            # percent_outliers = round((total_outliers/len(df_mahal))*100, 2)
            # total_spcontigs = len(df_mahal[df_mahal['spacer_presence']==1])
            # total_nonspcontigs = len(df_mahal[df_mahal['spacer_presence']==0])
            # percent_spcontigs = round((total_spcontigs/len(df_mahal))*100,2)
            # sp_outliers_count = len(df_mahal[(df_mahal['spacer_presence']==1) and (df_mahal['outlier']==1)])
            # nonsp_outliers_count = len(df_mahal[(df_mahal['spacer_presence']==0) and (df_mahal['outlier']==1)])
 
            
            # print(bin)
            # print('total contigs: ', len(df_mahal))
            # print('raw percent outliers: ', percent_outliers, '%')
            # print('percent spcontigs: ', percent_spcontigs, '%')
            # print('percentage of outliers that are spcontigs: ', round((sp_outliers_count/total_outliers)*100,2), '%')
            # print('percent of outliers that are nonspcontigs: ',round((nonsp_outliers_count/total_outliers)*100, 2), '%')
            # print('percent of spcontigs that are outliers: ', round((sp_outliers_count/total_spcontigs)*100,2), '%')
            # print('percent of non_spcontigs that are outliers: ', round((nonsp_outliers_count/total_nonspcontigs)*100,2), '%', '\n')
            ...
            # plt.savefig(f"{outdir}/python_figs/elbow_UMAP_{mapping_tool}_{bin}.pdf", bbox_inches='tight', format="pdf")  
            # plt.savefig(f"{outdir}/python_figs/elbow_UMAP_{mapping_tool}_{bin}.png", bbox_inches='tight', format="png")  
            # plt.show()
            ...
            g = sns.scatterplot(x=embedding[:, 0], y=embedding[:, 1], 
                        ...
                        legend="full", ax=ax)
            g.legend(title="Spacer Presence",loc='upper right',bbox_to_anchor=(1.3, 1.0), ncol=1)
            ...
            # fig.savefig(f"{outdir}/python_figs/UMAP_abund_{mapping_tool}_{bin}.pdf", bbox_inches='tight', format="pdf")  
            # fig.savefig(f"{outdir}/python_figs/UMAP_abund_{mapping_tool}_{bin}.png", bbox_inches='tight', format="png")  
            plt.show()
            

            '''PLOT 2, COLOR = LENGTH, SHAPE = SPACER PRESENCE'''
            fig, ax = plt.subplots(figsize=(7,5))
            g = sns.scatterplot(x=embedding[:, 0], y=embedding[:, 1], 
                        c=df_mahal['length'],cmap="Reds",
                        edgecolor="black", style=df_mahal['spacer_presence'], s=100,
                        legend="full", ax=ax)
            g.legend(title="Spacer Presence",loc='upper right',bbox_to_anchor=(1.3, 1.0), ncol=1)

            norm = plt.Normalize(df_mahal['length'].min(), df_mahal['length'].max())
            sm = plt.cm.ScalarMappable(cmap="Reds", norm=norm)
            sm.set_array([])

            cax = fig.add_axes([ax.get_position().x1+0.05, ax.get_position().y0, 0.02, ax.get_position().height / 2])
            ax.figure.colorbar(sm, cax=cax, label="Contig Length")

            ax.set_title(f'UMAP - {bin}', fontsize=20)
            # ax.suptitle("Abundance Profiles of Contigs", fontsize = 15)
            ax.set_xlabel('UMAP 1', fontsize=15)
            ax.set_ylabel('UMAP 2', fontsize=15)
            fig.savefig(f"{outdir}/python_figs/UMAP_length_{mapping_tool}_{bin}.pdf", bbox_inches='tight', format="pdf")  
            fig.savefig(f"{outdir}/python_figs/UMAP_length_{mapping_tool}_{bin}.png", bbox_inches='tight', format="png")  
            plt.show()
            
            '''PLOT 3, COLOR = OUTLIER, SHAPE=SPACER PRESENCE'''
            # fig, ax = plt.subplots(figsize=(7,5))
            # g = sns.scatterplot(x=embedding[:, 0], y=embedding[:, 1], 
            ...
            # ax.set_xlabel('UMAP 1', fontsize=15)
            # ax.set_ylabel('UMAP 2', fontsize=15)
            # # fig.savefig(f"{outdir}/python_figs/UMAP_outlier_{mapping_tool}_{bin}.pdf", bbox_inches='tight', format="pdf")  
            # # fig.savefig(f"{outdir}/python_figs/UMAP_outlier_{mapping_tool}_{bin}.png", bbox_inches='tight', format="png")  
            # plt.show()

            '''PLOT 4, COLOR = PVAL, SHAPE = SPACER PRESENCE'''
            # fig, ax = plt.subplots(figsize=(7,5))
            # g = sns.scatterplot(x=embedding[:, 0], y=embedding[:, 1], 
            ...
            # ax.set_xlabel('UMAP 1', fontsize=15)
            # ax.set_ylabel('UMAP 2', fontsize=15)
            # fig.savefig(f"{outdir}/python_figs/UMAP_pval_abund_{mapping_tool}_{bin}.pdf", bbox_inches='tight', format="pdf")  
            # fig.savefig(f"{outdir}/python_figs/UMAP_pval_abund_{mapping_tool}_{bin}.png", bbox_inches='tight', format="png")  
            # plt.show()

            '''PLOT 4, COLOR = DISTANCE, SHAPE = SPACER PRESENCE'''
            # fig, ax = plt.subplots(figsize=(7,5))
            # g = sns.scatterplot(x=embedding[:, 0], y=embedding[:, 1], 
            #             c=df_mahal['Mahalanobis'],cmap="Blues",
            #             edgecolor="black", style=df_mahal['spacer_presence'], s=100,
            #             legend="full", ax=ax)
            # g.legend(title="Spacer Presence",loc='upper right',bbox_to_anchor=(1.3, 1.0), ncol=1)
            ...
            # cax = fig.add_axes([ax.get_position().x1+0.05, ax.get_position().y0, 0.02, ax.get_position().height / 2])
            # ax.figure.colorbar(sm, cax=cax, label="Mahalanobis Distance")

            # ax.set_title(f'UMAP - {bin}', fontsize=20)
            # # ax.suptitle("Abundance Profiles of Contigs", fontsize = 15)
            # ax.set_xlabel('UMAP 1', fontsize=15)
            # ax.set_ylabel('UMAP 2', fontsize=15)
            # # fig.savefig(f"{outdir}/python_figs/UMAP_mahal_{mapping_tool}_{bin}.pdf", bbox_inches='tight', format="pdf")  
            # # fig.savefig(f"{outdir}/python_figs/UMAP_mahal_{mapping_tool}_{bin}.png", bbox_inches='tight', format="png")  
            # plt.show()

            plt.scatter(df_mahal['arrcpm'], df_mahal['length'])
            plt.show()

            plt.hist(df_mahal['length'], bins=30)
            plt.show()

            plt.hist(df_mahal['arrcpm'], bins=30)
            plt.show()
